...
import { WHATSAPP, WA_BASE } from "@/lib/contact";
...
export default function WhatsAppButton() {
  ...
  const handleClick = () => {
    if (items.length > 0) {
      ...
      const defaultMsg =
        "Hi Techno Machinery! 👋 I'd like to know more about your construction equipment — products, catalogue and pricing. Please share details.";
      window.open(`${WA_BASE}?text=${encodeURIComponent(defaultMsg)}`, "_blank");
    }
  };
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-center gap-2">
      {items.length > 0 && (
        <motion.div
          ...
          className="bg-[#071B45] text-[#F7B500] text-xs font-bold px-3 py-1.5 rounded-full border border-[#F7B500]/30 shadow-lg whitespace-nowrap"
        >
